...
from climate.utils import load_dataset, load_and_preprocess_highres_dataset, match_vectors_indices
...
results_folder = "/storage/homefs/ct19x463/Dev/Climate/reporting/high_res/results/"
...
dataset_mean_highres, dataset_members_highres = load_and_preprocess_highres_dataset(base_folder)
helper_filter_highres = EnsembleKalmanFilterScatter(dataset_mean_highres, dataset_members_highres, dataset_instrumental, client, is_zarr=False)


def run_single_month(assimilation_date):
    # Extract the (vectorized/stacked) data for the current month.
    mean_ds_highres = helper_filter_highres.dataset_mean.get_window_vector(assimilation_date, assimilation_date, variable_name='temperature')
    ensemble_ds_highres = helper_filter_highres.dataset_members.get_window_vector(assimilation_date, assimilation_date, variable_name='temperature')

    print(mean_ds_highres)
    print(ensemble_ds_highres)

    mean_ds_highres, ensemble_ds_highres = client.persist(mean_ds_highres), client.persist(ensemble_ds_highres)
    ...
    matched_inds = match_vectors_indices(mean_ds_highres, data_month_ds)
    
    # WARNING: Never try to execute bare loops in DASK, it will exceed the maximal graph depth.
    G = np.zeros((data_month_ds.shape[0], mean_ds_highres.shape[0]))
    ...
    raw_estimated_cov_lazy = ds.estimation.empirical_covariance(ensemble_ds_highres.chunk((1, 1800)))  
    ...
    grid_pts = da.vstack([mean_ds_highres.latitude, mean_ds_highres.longitude]).T
    ...
    mean_updated_aao, ensemble_updated_aao = my_filter.update_ensemble(
        mean_ds_highres.data, ensemble_ds_highres.data, G,
    ...
    unstacked_updated_mean_aao = helper_filter_highres.dataset_mean.unstack_window_vector(mean_updated_aao.compute(), time=assimilation_date, variable_name='temperature')
    unstacked_prior_mean = helper_filter_highres.dataset_mean.unstack_window_vector(mean_ds_highres.values.reshape(-1), time=assimilation_date, variable_name='temperature')

    # Unstack the ensembles too.
    print(ensemble_updated_aao.compute().shape)
    unstacked_updated_ensemble_aao = helper_filter_highres.dataset_members.unstack_window_vector(
            ensemble_updated_aao.compute(), time=assimilation_date, variable_name='temperature')
    
    return (unstacked_updated_mean_aao, unstacked_updated_ensemble_aao, unstacked_prior_mean)
...
for year in range(1969, 1970):
    for month in range(1, 2):
        ...
        # Run assimilation.
        (unstacked_updated_mean_aao, unstacked_updated_ensemble_aao, unstacked_prior_mean) = run_single_month(assimilation_date)
